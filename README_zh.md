...
- [x] Auto 布局：`layout = "auto"` / `--layout auto` 按流区宽度响应式 split→stack→unified
...
| `layout` | string | `"unified"` | `"unified"`（默认，交错显示）、`"stack"`（每文件旧/新上下两块）、`"split"`（左右分栏；流区 <80 列退化为 stack，<40 列退化为 unified）、或 **`"auto"`**（与 split 相同的响应式阶梯——推荐宽屏 / 可拖拽终端）。可用 `--layout` 覆盖。 |
