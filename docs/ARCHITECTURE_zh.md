...
| **Agent 时代** | 终端原生、可脚本化，面向人类与 coding agent |
...
| 正确、可扩展的 source / 高亮能力 | 为静态链接而拒绝合适库 |
...
- **对 agent 有用**：终端原生、可脚本化的纯文本输出，能接进任意管线
...
│ O(visible ± overscan)     │   │ 高亮 │ 搜索 │ watch            │
...
- **相对 delta**：在超大 diff 上的交互式多文件 review
...
| 风险 | 缓解 |
|------|------|
| 功能追着所有 git TUI 跑 | 锁死非目标；专精 review |
...
| Phase 4 | 「可测延迟、超大 diff 上顺滑的交互式 review」 |
