...
- [x] **UI 打磨**：文件头 A/M/D/R 徽章 + 右对齐 `+ins ███ −del` 比例条
  ...
  键列自适应（对标 hunk 并超越）
- [x] **性能证据**：同机实测写入 PERF.md/PERF_zh.md —— 启动基线
  2 ms vs 203 ms（~100×），TUI RSS 25.8 vs 115.7 MB（省 4.5×）、
  32.5 vs 177.8 MB（省 5.5×），视口 ~350 µs/40 行窗口；含方法与归因说明

**0.10 收尾（2026-08-31，#93–#97）：** hunk 0.20 主路径 29 项功能，
全量 ~24 项（>80%）；未追 4 项（extensions API、STML、自定义主题文件、
gh PR 扩展）均为 hunk experimental 边缘面，另有 file_gap/hunk_gap 因
上下文折叠已覆盖其视觉需求而有意不做。UI（变更徽章 + 比例条 + 树状
注释卡）与性能（启动 ~100×、RSS 4.5–5.5× 领先且有实测）超额达标。
